    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <upload-img ref="uploadPhoto" :default-url="employeesPic" @on-success="uploadPhotoSuccess" />
        </el-form-item>
      ...
      <div class="block">
        ...
        <el-row class="inline-info" type="flex" justify="center">
          <el-col :span="12">
            <el-button type="primary" @click="saveEmployessInfo">保存更新</el-button>
            <el-button @click="$router.back()">返回</el-button>
          </el-col>
        </el-row>
      </div>
...
import { getUserDetailById, getEmployeesInfo, saveUserDetailById } from '@/api/user'
import { saveEmployessInfo } from '@/api/employee'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    async saveEmployessInfo() {
      try {
        if (this.$refs.uploadPhotoSuccess.loading) {
          return this.$message.error('照片正在上传')
        }
        await saveEmployessInfo(this.formData)
